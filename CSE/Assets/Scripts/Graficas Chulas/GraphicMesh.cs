...
namespace Hexstar.Graphics
{
    public class GraphicMesh : MonoBehaviour
    {
        ...
        {
            if (datos != null) PrepararDatos(datos);
        ...
        {
            cRenderer.SetAlpha(1);
        }
        public void HideMesh()
        {
            cRenderer.SetAlpha(0);
        }

        public void CambiarMaterial(Material mat) { cRenderer.SetMaterial(mat, 0); }

        public void PrepararDatos(float[] datos, bool saveCopy = false)
        {
            if(saveCopy)
            {
                this.datos = (float[]) datos.Clone();
            }
