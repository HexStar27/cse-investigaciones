...
#pragma warning restore 0649
    ...
    public string logInUri = "http://localhost:8081/login";
    public string signInUri = "http://localhost:8081/signin";
    ...
    IEnumerator IniciarSesion(string correo, string contra)
    {
        ...
        if (correo.Contains("@uca.es") || correo.Contains("@alum.uca.es"))
        {
            ...
            //
            UnityWebRequest loginPage = UnityWebRequest.Get(logInUri);
            ...
            List<IMultipartFormSection> formulario = new List<IMultipartFormSection>
            {
                new MultipartFormDataSection("username=" + correo),
                new MultipartFormDataSection("password=" + CrearHash(contra)),
            };

            using (serverRequest = UnityWebRequest.Post(logInUri, formulario))
            {
                serverRequest.SetRequestHeader("referer", logInUri);
